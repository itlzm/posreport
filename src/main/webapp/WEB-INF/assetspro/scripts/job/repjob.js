...
require([ 'echarts', 'echarts/chart/bar', 'echarts/chart/line',
	...
	myChart1 = ec.init(document.getElementById('chart-2'));
	/**
	 * 
	option1 = {
		title : {
...
function fnBtnClickSetQuery() {
	...
	jobStep = $("#setJobStep").select2("val");
	if(jobStep == 'ALL'){
		jobStep = '';
	}
	...
	fnQueryData(BrandName, StoreName, ChannelName,jobStep);
...
// 查询数据
function fnQueryData(BrandName, StoreName, ChannelName,jobStep) {
	fnLoadingShowAndClose(true);
//	StoreCode = $("#StoreListVal").val();
	$.ajax({
		type : "POST",
		dataType : "json",
		url : baseUrl + '/job/queryJobDetail',
		data : {
		...
		if (i == 0) {
			...
			for (obj in listData[i]) {
				...
				if (obj == "日志编码") {
					...
					strColName += "<th scope='col' style='text-align: center;padding-left: 30px;padding-right: 80px;'>"
							+ obj + "</th>";
				} else {
...
function fnInitForm() {
	 ...
	 */
	//获取所有的执行步骤，将值设置到下拉选择空间中
	$.ajax({
		url:baseUrl + '/common/dict/jobStep',
		type:"get",
		dataType:"json",
		success:function(data){
			var jobStep = JSON.stringify(eval(data.jobStep));
			jobStep = jobStep.substring(1, jobStep.length);
			var jobStep = '[{"id":"ALL","text":"全部步骤"},'
					+ jobStep;
			$("#setJobStep").select2({
				data : eval(jobStep)
			});
			$("#setJobStep").select2("val", "ALL");
		},
		error:function(errMsg, errTextType, errorThrown){}
	});
 ...
 */
  ...
  function fnExportExcel() {
	...
	exportIframe.src = baseUrl + '/job/queryJobDetailExportExcel/' + rPam;
