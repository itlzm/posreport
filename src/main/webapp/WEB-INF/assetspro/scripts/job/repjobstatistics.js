...
function fnQueryData(BrandCode, StoreCode, ChannelCode, favourableName) {
	...
	$.ajax({
		type : "POST",
		dataType : "json",
		url : baseUrl + '/job/queryRepjobstatistics/',
		data : {
...
function fnDataTXHOne(dataTXHOne){
	...
	var strJobTime = "";
	var strJobTimeNum = "";
	//value为json块中每个json对象，jobStep和jobStepNum为json对象中每个值的key
	$.each(dataTXHOne, function(key,value) {		
		strJobTime += "'" + value.jobTime + "',";
		strJobTimeNum += "" + value.jobTimeNum + ",";
	});
			
	if (strJobTime != "") {
		strJobTime = strJobTime.substr(0,
				strJobTime.length - 1);
	}

	if (strJobTimeNum != "") {
		strJobTimeNum = strJobTimeNum.substr(0,
				strJobTimeNum.length - 1);
	}
	
	strJobTime = "[" + strJobTime + "]";
	strJobTimeNum = "[" + strJobTimeNum + "]";
	...
	for (var i = 0; i < listData.length; i++) {
		...
		$.each(listData[i], function(key, value) {
			if(key == '日期'){
				...
				strColNameValue += "<td align='center'><a href='JavaScript:void(0)' onclick=\"JavaScript:fnDownGo('"+value+"')\">"
					+ value + "</a></td>";
			}else{
				strColNameValue += "<td align='center'>"
					+ value + "</td>";
			}
//			if (isfloat(value)) {
//
...
//下钻方法，在日期上点击时，会重新发送请求，接受数据，并将数据显示在二维表格中

function fnDownGo(jobTime){
	//下钻时放入jobTime的值
	$("#jobTime").html(jobTime);
	
	$.ajax({
		url:baseUrl + '/job/queryDownGo',
		type:"POST",
		data:{
			jobTime:jobTime,
			PageIndex : pageIndex,
		...
		success : function(data) {
			...
			var pageData = data.pageData;
			//图形报表信息
			var dataTXH = data.dataTXH;
			fnDownGoDataTXH(dataTXH);
			// 合计行
			//var listDataSumTemp = data.listDataSum;
...
//				listDataSum = listDataSumTemp;
//			}
			fnMyListTableData(listData, pageData);
			fnLoadingShowAndClose(false);
		},
		error : function(e) {
			fnLoadingShowAndClose(false);
...
//下钻二级页面显示表格的详细信息，以及分页查询，参照fnListTableData
function fnMyListTableData(listData, pageData){
	...
	for (var i = 0; i < listData.length; i++) {
		strColNameValue += "<tr>";
		strColNameValue += "<td>" + ((pageIndex - 1) * pageSzie + i + 1) + "</td>"; // 明细行号列
	...
	$('#dataPager').bootpag({
		...
		pageIndex = num;
		var jobTime = $("#jobTime").text();//获取jobTime的值，再次调用fnDownGo方法，实现分页显示
		fnDownGo(jobTime);
	});
...
//下钻时显示图形化报表
function fnDownGoDataTXH(dataTXH){
	if (dataTXH.length == 0) {
		myChart1.clear();
		myChart1.setOption(option1);
		return;
	...
	// 获取步骤和步骤次数
	var len = dataTXH.length;
	var strJobStep = "";
	var strJobStepNum = "";
	//value为json块中每个json对象，jobStep和jobStepNum为json对象中每个值的key
	$.each(dataTXH, function(key,value) {		
		strJobStep += "" + value.jobStep + ",";
		strJobStepNum += "" + value.jobStepNum + ",";
	});
			
	if (strJobStep != "") {
		strJobStep = strJobStep.substr(0,
				strJobStep.length - 1);
	}

	if (strJobStepNum != "") {
		strJobStepNum = strJobStepNum.substr(0,
				strJobStepNum.length - 1);
	}
	
	strJobStep = "[" + strJobStep + "]";
	strJobStepNum = "[" + strJobStepNum + "]";
	...
	myChart1.hideLoading();
	myChart1.clear();
	myChart1.setOption(option2Data);
}
// 查询重置按钮点击执行方法
...
function fnExportExcel() {
	...
	exportIframe.src = baseUrl + '/job/queryRepjobstatisticsExportExcel/' + rPam;
