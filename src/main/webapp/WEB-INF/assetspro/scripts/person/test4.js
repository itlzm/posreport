...
function fnQueryData(BrandCode, StoreCode, ChannelCode, favourableName) {
	...
	$.ajax({
		type : "POST",
		dataType : "json",
		url : baseUrl + '/person/queryRepjobStatistics/',
		data : {
		...
		success : function(data) {
			...
			var ZZTData = data.ZZTData;
			var DBTData = data.DBTData;
			//fnPayModeData(BTNData, ZZTData);
			//fnPayModeSubSamountData(ZZTData);
...
function fnEdtBrandClick(strPrm) {
	...
	//后续修改：*****************************************************
	navAry[navAryL] = "BrandClick$" + strPrm + "$" + strPrm;
	//后续修改：*****************************************************
	var strHtml = "";
	...
	if (areaCode != "") {
		...
		$.ajax({
			...
			dataType : "json",
			//后续添加：下钻处理路径********************************************
			url : baseUrl + '/person/queryRepjobDetail/',
			//后续添加：下钻处理路径********************************************
			data : {
				//后续添加：strPrm为超链接传入jobTime的值。**********************
				JobTime : strPrm,
				//后续添加：strPrm为超链接传入jobTime的值。**********************
				TimeStart : dteTimeStart,
				TimeEnd : dteTimeEnd,
	...
	if (areaCode != "") {
		...
		if (isStore == "N") {
			...
			$.ajax({
				type : "POST",
				dataType : "json",
				url : baseUrl + '/activity/queryRepChannelActivityBrandChildren/',
				data : {
					TimeStart : dteTimeStart,
					TimeEnd : dteTimeEnd,
					BrandCode : brandCode,
			...
			$.ajax({
				type : "POST",
				dataType : "json",
				url : baseUrl + '/activity/queryRepChannelActivity/',
				data : {
					TimeStart : dteTimeStart,
					TimeEnd : dteTimeEnd,
					BrandCode : brandCode,
	...
	for (var i = 0; i < listData.length; i++) {
		...
		$.each(listData[i], function(key, value) {
			if (isfloat(value)) {
				...
				//后续添加：********************************************
					var valueAry = value.split("|=|");
					strColNameValue += "<td align='center'><a href='JavaScript:void(0)' onclick=\"JavaScript:fnEdtBrandClick('"+value+"')\">"
						+ value + "</a></td>";
				//后续添加：********************************************
				}else{
...
function fnExportExcel() {
	...
	//后续添加：jobTime。下钻时导出Excel使用******************************************
	var JobTime = $("#DataNav").children().eq(1).text();
	//后续添加：jobTime。下钻时导出Excel使用******************************************
	...
	//后续添加：****************************************************
	rPam += "&JobTime=" + JobTime;
	//后续添加：****************************************************
	var exportIframe = document.getElementById("exportIframe");
	exportIframe.src = baseUrl + '/person/queryRepjobStatisticsExportExcel/' + rPam;
