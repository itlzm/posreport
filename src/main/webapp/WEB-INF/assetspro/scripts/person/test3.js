...
function fnQueryData(BrandName, StoreCode, ChannelName) {
	...
	//后续添加：获取下拉控件中选取的步骤数。**************************************
	var jobStep = $("#setJobStep").select2("val");
	//后续添加：获取下拉控件中选取的步骤数。**************************************
	$.ajax({
		type : "POST",
		dataType : "json",
...
function fnExportExcel() {
	...
	//后需添加：导出的Excel表格显示执行步骤信息.*******************
	rPam += "&JobStep=" + JobStep;
	//后需添加：导出的Excel表格显示执行步骤信息.*******************
	var exportIframe = document.getElementById("exportIframe");
	exportIframe.src = baseUrl + '/person/queryRepjobDetailExportExcel/' + rPam;
