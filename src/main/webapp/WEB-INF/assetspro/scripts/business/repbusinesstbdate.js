...
function fnQueryData(BrandCode, StoreCode, ChannelName, favourableName) {
	...
	$.ajax({
		type : "POST",
		dataType : "json",
		url : baseUrl + '/business/queryRepBusinessTBDate/',
		data : {
...
function fnEdtBrandClick(strPrm) {
	...
	if (areaCode != "") {
		...
		$.ajax({
			type : "POST",
			dataType : "json",
			url : baseUrl + '/business/queryRepBusinessTBDateBrandChildren/',
			data : {
				TimeStart : dteTimeStart,
				TimeEnd : dteTimeEnd,
				BrandCode : brandCode,
	...
	if (areaCode != "") {
		...
		if (isStore == "N") {
			...
			$.ajax({
				...
				dataType : "json",
				url : baseUrl
						+ '/business/queryRepBusinessTBDateBrandChildren/',
				data : {
					TimeStart : dteTimeStart,
					TimeEnd : dteTimeEnd,
					BrandCode : brandCode,
			...
			$
					.ajax({
						...
						dataType : "json",
						url : baseUrl
								+ '/business/queryRepBusinessTBDateBrandChildrenStore/',
						data : {
							TimeStart : dteTimeStart,
							TimeEnd : dteTimeEnd,
							BrandCode : brandCode,
						...
						success : function(data) {
							/*
							 * // 支付方式结构分析 var storePay = data.storePay; var
							 * storePayType = data.storePayType;
...
function fnExportExcel() {
	...
	var queryText = encodeURI(encodeURI("查询日期:+" + dteTimeStart + "~"
			+ dteTimeEnd + "\n同比时段:" + dteTBTimeStart + "~" + dteTBTimeEnd
	...
	var rPam = "?TimeStart=" + dteTimeStart + "&TimeEnd=" + dteTimeEnd
			+ "&BrandName=" + BrandName + "&BrandCode=" + BrandName
			+ "&StoreCode=" + StoreCode + "&ChannelName=" + ChannelName
			+ "&queryText=" + queryText + "&TBTimeStart=" + dteTBTimeStart
			+ "&TBTimeEnd=" + dteTBTimeEnd;

	var exportIframe = document.getElementById("exportIframe");

	exportIframe.src = baseUrl
			+ '/business/queryRepBusinessTBDateBrandChildrenStoreExportExcel/'
			+ rPam;
...
var repbusinesstbdate = function() {
