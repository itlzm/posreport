...
function fnQueryData(BrandCode, StoreCode, ChannelCode) {
	...
	$.ajax({
		type : "POST",
		dataType : "json",
		url : baseUrl + '/store/queryRepSalesCup/',
		data : {
...
// 第一级别品牌点击
function fnEdtBrandClick(strPrm, isPageChange) {
	...
	if (areaCode != "") {
		...
		$.ajax({
			type : "POST",
			dataType : "json",
			url : baseUrl + '/store/queryRepSalesCupDetail/',
			data : {
				TimeStart : dteTimeStart,
				TimeEnd : dteTimeEnd,
				BrandCode : brandCode,
...
function fnListTableData(listData, pageData, listDataSum) {
	...
	$('#dataPager').bootpag({
		...
		} else if (pageChangeType == "detail") {
			fnEdtBrandClick(queryValue, "N");
		}
...
function fnExportExcel() {
	...
	exportIframe.src = baseUrl + '/store/queryRepSalesCupDetailExportExcel/'
			+ rPam;
