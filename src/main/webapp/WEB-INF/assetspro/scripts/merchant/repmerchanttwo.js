...
// 查询数据
function fnQueryData(BrandName, StoreCode, ChannelName, Paymentkey) {
	fnLoadingShowAndClose(true);
	StoreCode = $("#StoreListVal").val();
	//后续添加：********************************
	var fullName = $("#fullName").val().trim();
	var contractPerson = $("#contractPerson").val().trim();
	//后续添加：********************************
	$.ajax({
		type : "POST",
		dataType : "json",
		//url : baseUrl + '/store/queryReppayDetail/',
		//修改后：*************************************
		url : baseUrl + '/merchant/queryMerchantDetail2/',
		//修改后：*************************************
		data : {
			//后续添加：********************************
			FullName : fullName,
...
function fnExportExcel() {
	...
	if(ContractPerson == 'ALL'){
		ContractPerson = '';
	}
	rPam += "&FullName=" + FullName + "&ContractPerson=" + ContractPerson;
	//后续添加：商家名称和联系人名称。********************************
	var exportIframe = document.getElementById("exportIframe");
	exportIframe.src = baseUrl + '/merchant/queryMerchantDetailExportExcel2/' + rPam;
