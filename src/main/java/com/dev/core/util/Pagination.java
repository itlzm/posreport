...
public class Pagination extends JdbcDaoSupport {
	 ...
	 *            oracle语句
	 */
	public Pagination(String sql,String totalSQL,PageModel pageModel) {
		if (jTemplate == null) {
		...
		new Pagination(sql, currentPage, NUMBERS_PER_PAGE, jTemplate,totalSQL,pageModel);
	}
	 ...
	 *            JdbcTemplate实例
	 */
	public Pagination(String sql, int currentPage, int numPerPage, JdbcTemplate jTemplate,String totalSQL,PageModel pageModel) {
		if (jTemplate == null) {
			throw new IllegalArgumentException(
					"com.deity.ranking.util.Pagination.jTemplate is null,please initial it first. ");
		...
		setTotalRows(getJdbcTemplate().queryForObject(totalSQL, Integer.class));
